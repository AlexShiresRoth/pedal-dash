...
const InitialResult = ({ loading, pedalRes, pedalIndex, setIndex }) => {
  const changeIndexNext = () => {
    return pedalIndex === pedalRes.listings.length - 1
      ? setIndex((pedalIndex = 0))
      : setIndex((pedalIndex += 1));
  };

  const changeIndexBack = () => {
    return pedalIndex !== 0
      ? setIndex((pedalIndex -= 1))
      : setIndex(pedalRes.listings.length - 1);
  };

  const svgWave = (
    <svg
      className={resStyles.wave}
  ...
  const pedalKnob = (
    <svg
      className={resStyles.pedal__knob}
      viewBox="0 0 100 100"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g>
  ...
  const removeHtmlTags = content => {
    ...
    return content.replace(
      /<strong>|<\/strong>|<ul>|<\/ul>|<b>|<\/b>|<p>|<\/p>|<br>|<\/br>/g,
      ""
    );
  };
    ...
    <div className={resStyles.pedal__container}>
      ...
      <div className={resStyles.column}>
        ...
        <div className={resStyles.container}>
          ...
          <p>
            {removeHtmlTags(pedalRes.listings[pedalIndex].description).length >
            200
              ? removeHtmlTags(
                  pedalRes.listings[pedalIndex].description
                ).substr(0, 200) + "..."
        ...
        <div className={resStyles.container}>
          ...
          <span>Year:</span>
          <p>
            {pedalRes.listings[pedalIndex].year !== ""
              ? pedalRes.listings[pedalIndex].year
              : "N/A"}
          </p>
        </div>
